...
int perturbations_init(
  ...
  if (ppt->has_tensors == _TRUE_) {
    ...
    /*
      - In the case tm_photons_only, we do not want to evolve ur and ncdm perturbations, even if they are present at the background level.
      - In the other two cases, we want to account for ur perturbations and/or ncdm perturbations, in one way or another.
        The difference is that:
            - with tm_exact, we compute the contribution to delta T_munu from ur perturbations if present, from ncdm perturbations if present,
              and we add them up if they are both present.
            - with tm_massless_approx, we treat the ncdm perturbations in a simplified way, with the same equations as for ur perturbations.
              The contribution to delta T_munu is fomally equivalent to that of ur, but normalised to the sum (rho_ur+rho_ncdm).
              In this case, we don't need to follow ncdm perturbations.
              However, we always need to follow ur perturbations, even when at the background level there is some ncdm but no ur.

      Thus:
      - ppt->evolve_tensor_ur = _TRUE_ only implies that at least one of pba->has_ur or pba->has_ncdm is _TRUE_
      - ppt->evolve_tensor_ncdm = _TRUE_ always implies pba->has_ncdm = _TRUE_
     */

    /* ppt->evolve_tensor_ur and ppt->evolve_tensor_ncdm where set by default to _FALSE_ in input.c */
    ...
    case (tm_massless_approximation):
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_),
                 ppt->error_message,
                 "For tensor modes, the method tm_massless_approximation requires either ur or ncdm to be present.");

      /* in any case we will only follow tensor ur perturbations */
    ...
    case (tm_exact):
      ...
      class_test((pba->has_ur == _FALSE_) && (pba->has_ncdm == _FALSE_),
                 ppt->error_message,
                 "For tensor modes, the method tm_exact requires either ur or ncdm to be present.");

      /* we follow tensor ur perturbations if ur is present at the background level
         and/or tensor ncdm perturbations if ncdm is present at the background level */
      if (pba->has_ur == _TRUE_)
        ppt->evolve_tensor_ur = _TRUE_;
      if (pba->has_ncdm == _TRUE_)
  ...
  if (ppt->has_vectors == _TRUE_) {
    ...
    /*
      The vectors perturbation methods are similar to the tensor one, except that there is no equivalent to tm_photons_only.
      One always needs either ur species or ncdm species, because the early free-streaming of these particles sustains the vector metric potential.
      We want to account for ur perturbations and/or ncdm perturbations, in one way or another.
            - with vm_exact, we compute the contribution to delta T_munu from ur perturbations if present, from ncdm perturbations if present,
              and we add them up if they are both present.
            - with vm_massless_approx, we treat the ncdm perturbations in a simplified way, with the same equations as for ur perturbations.
              The contribution to delta T_munu is fomally equivalent to that of ur, but normalised to the sum (rho_ur+rho_ncdm).
              In this case, we don't need to follow ncdm perturbations.
              However, we always need to follow ur perturbations, even when at the background level there is some ncdm but no ur.

      Thus:
      - ppt->evolve_vector_ur = _TRUE_ only implies that at least one of pba->has_ur or pba->has_ncdm is _TRUE_
      - ppt->evolve_vector_ncdm = _TRUE_ always implies pba->has_ncdm = _TRUE_
     */
    ...
    /* ppt->evolve_vector_ur and ppt->evolve_vector_ncdm were set by default to _FALSE_ in input.c */

    switch (ppt->vector_method) {

    case (vm_massless_approximation):

      /* in any case we will only follow vector ur perturbations */
      ppt->evolve_vector_ur = _TRUE_;
      break;

    case (vm_exact):

      /* we follow vector ur perturbations if ur is present at the background level
         and/or vector ncdm perturbations if ncdm is present at the background level */
      if (pba->has_ur == _TRUE_)
        ppt->evolve_vector_ur = _TRUE_;
      if (pba->has_ncdm == _TRUE_)
...
int perturbations_indices(
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_vectors_) {
      ...
      class_define_index(ppt->index_tp_vector_t1,      ppt->has_source_t,index_type,1);
...
int perturbations_get_k_list(
  ...
  int index_k, index_k_output, index_md;
  ...
  for (index_md=0; index_md < ppt->md_size; index_md++){
    ...
    if (_scalars_) {
      ...
      l_max = ppt->l_tensor_max;
        ...
        class_stop(ppt->error_message,"unidentified mode index %d",index_md);
      }
    ...
    if (pba->sgnK == 0) {
      ...
      /* K>0 (closed): start from q=sqrt(k2+(1+m)K) equal to 3sqrt(K), i.e. k=sqrt((8-m)K), minus a tiny decrement */
      k_min = sqrt((8.-m-1.e-4)*pba->K);
    ...
    /** - --> find k_max (as well as k_max_cmb[index_md], k_max_cl[index_md]) */

    k_rec = 2. * _PI_ / pth->rs_rec; /* comoving scale corresponding to sound horizon at recombination */

    k_max_cmb[index_md] = k_min;
    k_max_cl[index_md] = k_min;
    k_max = k_min;

    if (ppt->has_cls == _TRUE_) {

      /* find k_max_cmb[index_md] : */

      /* choose a k_max_cmb[index_md] corresponding to a wavelength on the last
         scattering surface seen today under an angle smaller than
         pi/lmax: this is equivalent to
         k_max_cl[index_md]*[comvoving.ang.diameter.distance] > l_max */

      k_max_cmb[index_md] = ppr->k_max_tau0_over_l_max*l_max
        /pba->conformal_age/pth->angular_rescaling;
      k_max_cl[index_md] = k_max_cmb[index_md];
      k_max     = k_max_cmb[index_md];

      if (_scalars_) {

        /* find k_max_cl[index_md] : */
        ...
        /* if we need density/lensing Cl's, we must impose a stronger condition,
           ...
           k_max_cl[index_md] by the ratio tau0/(tau0-tau[center of smallest
        ...
        if ((ppt->has_cl_number_count == _TRUE_) || (ppt->has_cl_lensing_potential == _TRUE_)) {
          ...
          k_max_cl[index_md] = MAX(k_max_cl[index_md],ppr->k_max_tau0_over_l_max*ppt->l_lss_max/(pba->conformal_age-tau1)); // to be very accurate we should use angular diameter distance to given redshift instead of comoving radius: would implement corrections depending on curvature
          k_max    = k_max_cl[index_md];
        }
      }
    }
    ...
    if (_scalars_) {
      ...
      /* find overall k_max including P(k) and CMB lensing requirements: */
    ...
    index_k=0;
    k = k_min;
    ppt->k[index_md][index_k] = k;
    index_k++;

    /* values until k_max_cmb[index_md] */

    while (k < k_max_cmb[index_md]) {
      ...
      class_test(k <= ppt->k[index_md][index_k-1],
                 ppt->error_message,
    ...
    if (_scalars_) {
      ...
      /* values until k_max_cl[index_md] */

      while (k < k_max_cl[index_md]) {
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    for (index_md=0; index_md<ppt->md_size; index_md++){
      ...
      newk_size = ppt->k_size[index_md]+ppt->k_output_values_num;
      ...
      for (index_newk=0; index_newk<newk_size; index_newk++){
        ...
        if (add_k_output_value == _TRUE_){
          ...
          ppt->index_k_output_values[index_md*ppt->k_output_values_num+index_k_output]=index_newk;
          index_k_output++;
        }
        else{
          tmp_k_list[index_newk] = ppt->k[index_md][index_k];
          index_k++;
        }
      }

      free(ppt->k[index_md]);
      ppt->k[index_md] = tmp_k_list;
      ppt->k_size[index_md] = newk_size;

      index_k = newk_size-1;
      while (ppt->k[index_md][index_k] > k_max_cl[index_md])
        index_k--;
      ppt->k_size_cl[index_md] = MIN(index_k+2,ppt->k_size[index_md]);

      index_k = newk_size-1;
      while (ppt->k[index_md][index_k] > k_max_cmb[index_md])
        index_k--;
      ppt->k_size_cmb[index_md] = MIN(index_k+2,ppt->k_size[index_md]);
  ...
  if (_vectors_) {
    ...
    case optimal:
      ...
      if (ppt->evolve_vector_ur == _TRUE_)
        ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ur_vec);
      break;
  ...
  if (_tensors_) {
    ...
    case optimal:
      ...
      if (ppt->evolve_tensor_ur == _TRUE_)
        ppw->max_l_max = MAX(ppw->max_l_max, ppr->l_max_ur_ten);
      break;
...
int perturbations_solve(
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    if (is_early_enough == _TRUE_) {
      ...
      /* For vector perturbations the condition to decide whether we start early enough can be stronger than for scalars.
         Below, om is the quantity $\omega$ defined in 3.2 of 2410.03612, that is om = {\cal H}_0 \Omega_m/\sqrt{\Omega_r}.
         ...
         we must make sure that om * tau << 1 by choosing om*tau < ppr->start_vectors_at_omega_tau << 1.
      */
      if (_vectors_) {
        /* 8piG/3 rho_m(t_i) */
        ...
        /* {\cal H}_0 \Omega_m/\sqrt{\Omega_r} */
        om = ppw->pvecback[pba->index_bg_a]*rho_m/sqrt(rho_r);

        if (om*tau_mid > ppr->start_vectors_at_omega_tau)
          is_early_enough = _FALSE_;
      }
    }
...
/**
 * Updates the streaming coefficients which couple the multipoles of
 * photons and neutrinos They depend on the mode k, hence they must be
 * computed at the beginning of the time integration for each mode.
 *
 * @param pba        Input: pointer to background structure
 * @param ppt        Input: pointer to the perturbation structure
 * @param index_md   Input: index of mode under consideration (scalar/.../tensor)
...
int perturbations_vector_init(
  ...
  double rho_plus_p_ncdm,q,q2,epsilon,a,factor,R,R_Hu;
  ...
  if (_vectors_) {
    ...
    /* ncdm are integrated in all cases. Hence whenever we have one ncdm species, the code is much slower as these never enjoy the RSA approximation */
    if (ppt->evolve_vector_ncdm == _TRUE_) {
  ...
  if (_vectors_) {
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) { /* if radiation streaming approximation is off */
      ...
      /* We also do not need neutrino multipoles (which exist only when RSA is off),
         except for transfers of velocities, hence we keep l=0 and l=2 in optimal and l=1 in TAM. */
    ...
    /* we don't need ncdm multipoles
       unless one day we decide to output their dipole in output_tk,
       which will require to keep l=0,1,2 in optimal hierarchy. */
  ...
  if (_tensors_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) { /* if radiation streaming approximation is off */
      ...
      /* We do not need ur multipoles (which exist only when RSA is off)
         unless one day we decide to output their velocity in output_tk. */
    ...
    /* We also do not need ncdm multipoles
       unless one day we decide to output their velocity in output_tk. */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_vectors_) {
      ...
      /* There are two definitions of R:
         - In Ma & Berstchinger (astro-ph/9506072, equation 70), R = (4/3) (rho_g/rho_b)
           This is what we use by default in CLASS, in particular for scalar modes.
         - in Hu & Sujiyama, R is the inverse of the previous definition (e.g. astro-ph/9702170, equation 67)
           This is what is used by default in CAMB.
         CP: we use the second definition in CLASS only for vector modes, to match e.g. astro-ph/0403583.
         To avoid ambiguity, we call this one R_Hu.
      */

      R_Hu = 3./4. * ppw->pvecback[pba->index_bg_rho_b]/ppw->pvecback[pba->index_bg_rho_g];
      ...
      */
      if ((pa_old[ppw->index_ap_tca] == (int)tca_on) && (ppw->approx[ppw->index_ap_tca] == (int)tca_off)) {
        ...
        /* Since in TCA theta_b stands for the tight coupled fluid velocity (aka vorticity),
           we deduce baryons and photons velocity in the following manner.
           First \f$ dv = v_b - v_{photons} \f$ is given by 2.36 of 2410.03612 in TCA.
           We then combine 2.34 and 2.31 to deduce that \f$ v_{baryons} = v_{tcfluid} + 1/(1+R) dv \f$
           and \f$ v_{photons} = v_{tcfluid} -R/(1+R) dv \f$ (set a few lines below).
           After TCA is switched off, theta_b is truely the baryon velocity,
           hence this justifies the following rule at the end of TCA.
        */
        ppv->y[ppv->index_pt_theta_b] =
          ppw->pv->y[ppw->pv->index_pt_theta_b] + 1./(1.+R_Hu)*ppw->tca_slip_vector;

        switch (ppt->hierarchy) {
        case optimal:
          ppv->y[ppv->index_pt_l0_g] = -2.*_SQRT2_*(ppw->pv->y[ppw->pv->index_pt_theta_b]-R_Hu/(1.+R_Hu)*ppw->tca_slip_vector);
          /* We relate to the TAM methdo using B.15 of 1305.3261, evaluate for l=1,
             and without the last term which vanishes in tight coupling.
             Hence \f$ \Theta_1^{(1)} = -1/(2 \sqrt{2}) * F_0^{(1)} \f$. */
          ppv->y[ppv->index_pt_l0_g+1] = -4./(_SQRT6_)*ppw->tca_T2_vector;
          /* We also use B.15 of 1305.3261 for l=2,
             ignoring the last term which vanishes in tight coupling.
             Hence we get \f$ \Theta_2^{(1)} = -\sqrt{6}/4 F_1^{(1)} \f$ and
             we use the TAM result for \f $\Theta_2^{(1)} \f$ in tight-coupling. */
          ppv->y[ppv->index_pt_pol0_g] = -0.5 * _SQRT6_  *ppw->tca_T2_vector;
          /* We use B.22 of 1305.3261 neglecting the last two terms in the r.h.s in tight coupling.
             Hence \f$E_2^{(1)} = 1/2 G_0^{(1)} \f$.
             Then we also use the relation in tight coupling
             \f$ E_2^{(1)}  = -\sqrt{3/8}\Theta_2^{(1)} \f$.
          */
          break;
        case tam:
          ppv->y[ppv->index_pt_l1_g] = ppw->pv->y[ppw->pv->index_pt_theta_b]-R_Hu/(1.+R_Hu)*ppw->tca_slip_vector;
          /* See comment above for this rule as we deduce photon velocity from velocity slip
             and velocity of the tight-soupled fluid. */
          ppv->y[ppv->index_pt_l1_g+1] = ppw->tca_T2_vector;
          /* This is the definition of tca_T2_vector, namely the tight-coupling solution
             of photon quadrupole. */
          ppv->y[ppv->index_pt_E2] = - 0.25 * _SQRT6_ *ppw->tca_T2_vector;
          /* This relation is deduced by taking the tight-coupled limit of the hierarchy,
             that is 2.24 of 2410.03612 or 34 of astro-ph/9709066. */
          break;
        }
      }
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  double Phi0,R_Hu,om,cH;
  double V_init=0., theta_b=0., l1_ur=0., l2_ur=0., l4_ur=0.;
  ...
  if (_vectors_) {
    ...
    */
    ...
    if ((ppt->has_iso_v == _TRUE_) && (index_ic == ppt->index_ic_iso_v)) {
      ...
      /* In TCA theta_b stands for the velocity (akak vorticity) of the fluid, hence 3.8 of 2410.03612. */
      theta_b = Phi0/(1.+R_Hu) * (5.*rho_r + 4.*rho_nu) / (4.*ppw->pvecback[pba->index_bg_rho_g]);
      /* If not synchronous gauge we must add V to the velocities */
      if (ppt->gauge == newtonian) {
        l1_ur += V_init;
        theta_b += V_init;
      }
      /* Quadrupole of neutrinos, 3.7 of 2410.03612 */
      l2_ur = -Phi0 *k*_SQRT3_*ssqrt3 *(5./12*rho_r/rho_nu*tau+ cH/6.*tau*tau);
      /* l=3 is not excited for small k corresponding to q/sqrt(K) = 3, since in that case l_max = 2. */
      if ((pba->K <= 0.) || (k*k > 7.*pba->K))
        /* Octupole of neutrinos. C.Pitrou private Mathematica notebook. */
        l3_ur = -Phi0 /(2.*_SQRT6_) * sqrt((k*k -2.*pba->K) * (k*k -7.*pba->K)) *rho_r/rho_nu *tau*tau;
      /* l=4 stays null. */
    }
    ...
    if ((ppt->has_oct_v == _TRUE_) && (index_ic == ppt->index_ic_oct_v)) {
      ...
      /* Eq. 3.27 */
      V_init = Phi0 *(1.- cH*tau);
      /* Eq. 3.28, dipole of neutrinos */
      l1_ur = Phi0/8.*(k*k -2.*pba->K)*rho_r/rho_nu*tau*tau;
      /* Eq 3.31. In TCA theta_b stands for the velocity of the tight coupled fluid. It stays null. */
      /* If not synchronous gauge we must add V to the velocities */
      if (ppt->gauge == newtonian) {
        l1_ur += V_init;
        theta_b += V_init;
      }
      /* Quadrupole of neutrinos Eq. 3.29 */
      l2_ur = -Phi0 *k*_SQRT3_*ssqrt3 *(5./12*rho_r/rho_nu*tau+ cH/6.*tau*tau);
      /* l=3 is not excited for small k corresponding to q/sqrt(K) = 3, since in that case l_max = 2. */
      if ((pba->K <= 0) || (k*k > 7.*pba->K))
        /* Eq. 3.30 for octupole of neutrinos */
        l3_ur = Phi0 * 7./3. * (_SQRT6_/4.) *sqrt((k*k -2.*pba->K) / (k*k -7.*pba->K))
          * (5.*rho_r + 4.*rho_nu) / (4.*rho_nu);
      /* l=4 is not excited for small k corresponding to q/sqrt(K) = 4, since in that case l_max = 3. */
      if ((pba->K <= 0) || (k*k > 14.*pba->K))
        /* C. Pitrou private notebook */
        l4_ur = Phi0*(5.*rho_r+4.*rho_nu)/rho_nu /8.* _SQRT5_/_SQRT2_ *sqrt((k*k -2.*pba->K) * (k*k -14.*pba->K) / (k*k -7.*pba->K))*tau;
    }

    /* Assign IC to all variables but ur and ncdm in synchronous gauge */
    ...
    /* From synchronous to Newtonian gauge, add V to the velocities */
    if (ppt->gauge == newtonian) {
      l1_ur += V_init;
      ppw->pv->y[ppw->pv->index_pt_theta_b] += V_init;
    }

    /* Assign IC to ur and ncdm variables */

    if (ppt->evolve_vector_ur == _TRUE_){

      switch (ppt->hierarchy) {
        /* We invert B.15 in 1305.3261, which is the same for neutrinos, and ignoring F_l^(1) for l>=4. */
      case optimal:
    ...
    if (ppt->evolve_tensor_ncdm == _TRUE_){
      ...
      for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
        for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
          ...
          // CP: I have guessed this from the hierarchy for ur species.
...
/**
 * Compute total stress-energy tensor (bacvkground and perturbations)
 *
 * @param ppr        Input: pointer to precision structure
 * @param pba        Input: pointer to background structure
 * @param pth        Input: pointer to thermodynamics structure
...
int perturbations_total_stress_energy(
  ...
  if (_vectors_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) { /* if radiation streaming approximation is off */
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) { /* if tight-coupling approximation is off */
        ...
        switch (ppt->hierarchy) {
        case optimal:
          /* This is deduced from the TAM result below, using B.15 of 1305.3261 that is with \f$ \Theta_2^{(1)} = - \sqrt{6}/4 (F_1^{(1)}+F_3^{(3)}) \f$. */
          ppw->vector_source_pi += -1./3.*a2*ppw->pvecback[pba->index_bg_rho_g]
            * (6.*_SQRT2_/5./ssqrt3)
            * (y[ppw->pv->index_pt_l0_g+1]+y[ppw->pv->index_pt_l0_g+3]);
          break;
        case tam:
          /* 2.21 of 2410.03612 is only for the flat case.
             For curved case we use Eq. 40 of astro-ph/9709066.
             The fancy (1-2K/k^2) factor for vectors (and all other factors in Eq. 40 of astro-ph/9709066)
             can be understood with Eq. 7.11 of 1909.13687, which leads for photons to
             \f$ \pi^{(m)}  = \frac{8}{5} P \Theta_2^{(m)} /( {}_0\tilde{g}^{(2m)} )\f$.
             For vectors we then use m=1 and
             \f$ {}_0 \tilde{g}^{21} = (1/\sqrt{3})\sqrt{q^2-4K}/k = (1/sqrt{3}) \sqrt{1 -2 K/k^2} \f$
             to deduce
             \f$ \pi^{(1)} = P * 8/5 \sqrt{3}/\sqrt{1-2K/k^2} \Theta_2^{(1)}\f$
          */
          ppw->vector_source_pi += 1./3.*a2*ppw->pvecback[pba->index_bg_rho_g]
            * (8.*_SQRT3_/5./ssqrt3)
        ...
        /* CP: If the TCA is on, we could still compute the
           anisotropic stress and the source for vector modes. However
           the contribution to the source from these times would be
           completely negligible. For simplicity, we stick to the
           approximation vector_source_pi = 0 during the TCA. */
      }
    }
    ...
    if (ppt->evolve_vector_ur == _TRUE_){
      ...
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        ...
        /* In the vm_exact method, we compute here the contribution from ur (if any) here.
           We will separately compute the contribution of ncdm (if any) later. */
        if ((pba->has_ur == _TRUE_) && (ppt->vector_method == vm_exact))
          rho_fs += ppw->pvecback[pba->index_bg_rho_ur];

        /* In the vm_massless approximation method, we build up an effective contribution
           formally equivalent to that of ur, but normalised to the sum of ur and ncdm. */
        ...
        switch (ppt->hierarchy) {
        case optimal:
          /* These expressions are explained above for photons. They are similar for neutrinos. */
    ...
    if (ppt->evolve_vector_ncdm == _TRUE_){
      /* Note that ppt->evolve_vector_ncdm = _TRUE_ always implies pba->has_ncdm = _TRUE_ */

      /* Note also that for ncdm we always use the optimal temperature
         hierarchy, not the tam temperature hierarchy. In principle
         this could lead to tiny differences for large |Omega_k-1| when
         the option "vector_method = vm_exact" is selected. In
      ...
      for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        /* Again similar to what was done for photons and neutrinos above. */
        pincdm *= -1./3.*a2*(6.*_SQRT2_/5./ssqrt3) *factor;
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_){
      ...
      /* In the tm_exact method, we compute here the contribution from ur (if any) here.
         We will separately compute the contribution of ncdm (if any) later. */
      if ((pba->has_ur == _TRUE_) && (ppt->tensor_method == tm_exact))
        rho_relativistic += ppw->pvecback[pba->index_bg_rho_ur];

      /* In the tm_massless approximation method, we build up an effective contribution
         formally equivalent to that of ur, but normalised to the sum of ur and ncdm. */
    ...
    if (ppt->evolve_tensor_ncdm == _TRUE_){
      /* Note that ppt->evolve_tensor_ncdm = _TRUE_ always implies pba->has_ncdm = _TRUE_ */
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_scf == _TRUE_) {
      ...
      _set_source_(ppt->index_tp_delta_scf) = delta_rho_scf;
    }
  ...
  if (_vectors_) {
    ...
    */
    ...
    R_Hu = 3./4. * pvecback[pba->index_bg_rho_b]/pvecback[pba->index_bg_rho_g];
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      /* TCA approximation. There is no published reference. */
      else {
        if (ppt->gauge == newtonian)
        ...
        /* We remind that theta_b is a proxy for the coupled fluid. It is not the baryon velocity in tight coupling. */
          ...
          /* Idem here theta_b as a proxy for tight-coupled fluid velocity. */
          P = -1./15. * k*ssqrt3/_SQRT3_ /ppw->pvecthermo[pth->index_th_dkappa]
            * (y[ppw->pv->index_pt_theta_b] + y[ppw->pv->index_pt_V]);
        }
        /* We infer the baryon and photon velocity from the coupled fluid and the slip velocity (see 2410.03612 section 2.2.4) */
        theta_g = (y[ppw->pv->index_pt_theta_b] - R_Hu/(1.+R_Hu)*ppw->tca_slip_vector);
        theta_b = (y[ppw->pv->index_pt_theta_b] +1./(1.+R_Hu)*ppw->tca_slip_vector);
      }
    }
    ...
    if (ppt->has_source_t == _TRUE_) {
      if (ppt->gauge == synchronous) {
        ...
        /* WARNING. We DO NOT use Eq. (36) of astro-ph/9709066 as a source for the hierarchy.
           Indeed, in synchronous gauge we also choose the photon dipole to be in the
           synchronous gauge, that is, we considern\f$ \Theta_1^{synchronous} = \Theta_1 - V\f$
           in the hierarchy (but all \f$\Theta_l\f$ for l=!=1 remain the same)
           and this modifies the source terms.
           The benefit is that in this gauge the metric vector perturbation appears directly,
           and not its time derivative.
           The source terms can be read from 7.25 of 1909.13687 (with B -> -V) but otherwise
           it can also be deduced by the replacement above in the Newtonian gauge hierarchy.
           This absorbs the metric perturbation in the $l=1$ equation but then generate
           a new contribution in the $l=2$ hierarchy since it couples to $l=1$.
        */

        /* l=1 and m=1 source */
        _set_source_(ppt->index_tp_vector_t1) = pvecthermo[pth->index_th_g]*theta_b;
        /* l=2 and m=1 source */
        ...
        /* l=1 and m=1 source */
        _set_source_(ppt->index_tp_vector_t1) = ppw->pvecmetric[ppw->index_mt_V_prime]*pvecthermo[pth->index_th_exp_m_kappa]
    ...
    /* vector polarisation source */
    if (ppt->has_source_p == _TRUE_) {

      /* Note that the correct source should have a minus sign, as
         shown in Hu & White. We use a plus sign to comply with the
         'historical convention' established in CMBFAST and CAMB for
         scalars and tensors. Since we adopted this convention for
         scalars and tensors, we stick to it for vectors. */
    ...
    if (ppt->has_source_vector_theta_ur == _TRUE_) {
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        ...
        /* if RSA is on we set the newtonian velocity to 0. */
        if (ppt->gauge == synchronous) {
    ...
    if (ppw->approx[ppw->index_ap_rsa]==(int)rsa_off) {
      if (ppw->approx[ppw->index_ap_tca]==(int)tca_off) {
        ...
        */
        switch (ppt->hierarchy) {
        case optimal:
          l0_g = -2*_SQRT2_*(y[ppw->pv->index_pt_theta_b] - R_Hu/(1.+R_Hu)*ppw->tca_slip_vector);
          ...
          break;
        case tam:
          l1_g = y[ppw->pv->index_pt_theta_b] - R_Hu/(1.+R_Hu)*ppw->tca_slip_vector;
...
int perturbations_derivs(double tau,
  ...
  double a,a2,a_prime_over_a,R,R_Hu;
  ...
  /* There are two definitions of R:
       - In Ma & Berstchinger (astro-ph/9506072, equation 70), R = (4/3) (rho_g/rho_b)
         This is what we use by default in CLASS.
       - in Hu & Sujiyama, R is the inverse of the previous definition (e.g. astro-ph/9702170, equation 67)
         This is what is used by default in CAMB.
    */
    ...
    */
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      if (ppw->approx[ppw->index_ap_tca]==(int)tca_off) {
        ...
        case optimal:
          ...
          if (ppt->gauge == synchronous) {
            ...
            dy[pv->index_pt_theta_b] = -(1-3.*cb2)*a_prime_over_a*y[pv->index_pt_theta_b]
              - pvecthermo[pth->index_th_dkappa]*(y[pv->index_pt_theta_b]-theta_g)/R_Hu;
          }
          else if (ppt->gauge == newtonian) {
          ...
          if (ppt->gauge == synchronous) {
            /* photon density (delta_g = F_0). 2.35 of 1305.3261 for l=0 m=1 remembering that s_1 = 1.
               Carefull that first expression for $u_0^{(1)}$ seems to have a typo.
               But its specifications for Newtonian and synchronous gauges are correct. */
          ...
          /* additional momenta in Boltzmann hierarchy.
             2.35 of 1305.3261 for general l>=2 and m=1 with u_l=0.*/
          ...
          /* photon polarization, l=0 (pol0_g = G_0), eq. (2.36) with m=1
             of 1305.3261, coming from (2.15b) of the same paper. */
          dy[pv->index_pt_pol0_g] =
            -k*y[pv->index_pt_pol0_g+1]
            -pvecthermo[pth->index_th_dkappa]*(y[pv->index_pt_pol0_g] + 2*_SQRT6_*P1);
          /* Note that in 1305.3261 this equation is correct at the
             level of (2.15b) (before the multipole expansion), but
             there is a sign mistake when it is re-written in the form
             of (2.36) with the definition of v_0^(1) given a few
             lines below. Here we use the correct sign for the term
             2*_SQRT6_*P1, consistent with (2.15b) and opposite to
             (2.36). */
        ...
        case tam:
          ...
          if (ppt->gauge == synchronous) {
            ...
            dy[pv->index_pt_theta_b] = -(1-3.*cb2)*a_prime_over_a*y[pv->index_pt_theta_b]
              - pvecthermo[pth->index_th_dkappa]*(y[pv->index_pt_theta_b]-y[pv->index_pt_l1_g])/R_Hu;
          }
          else if (ppt->gauge == newtonian) {
            /* Translation to Newtonian gauge. */
            dy[pv->index_pt_theta_b] = -(1-3.*cb2)*a_prime_over_a*y[pv->index_pt_theta_b]
              - pvecthermo[pth->index_th_dkappa]*(y[pv->index_pt_theta_b]-y[pv->index_pt_l1_g])/R_Hu
              + pvecmetric[ppw->index_mt_V_prime]+(1.-3.*cb2)*a_prime_over_a*y[pv->index_pt_V];
          }
          ...
          /* Hierarchy in e.g. astro-ph/9709066 for m=1 but in synchronous gauge
             we use \f$ \Theta_1^{sync} = \Theta_1^{newt} - V\f$.
             Hence this modifies the l=1 and l=2 equations. */
      ...
      /* TCA approximation */
        ...
        /* We recall that in TCA the baryons stand in fact for the tight-coupled fluid of
           baryons and photons. There is nothing different in the TAM hierarchy. */
        if (ppt->gauge == synchronous) {
          /* It looks like 3.32 in 2410.03612 but it is never really written for curved space
             in litterature. The tight-coupling of photon quadrupole is
             \f$ \Theta_2 = 1/\kappa'*(4/9 * {}_0\kappa_2^m ) \Theta_1^{newt} = 1/\kappa'*(4/9) * k * \sqrt{3*(1 - 2K/k^2)} \f$
             In the r.h.s we must then put tight coupled solution in newtonian gauge in place of
             \f$ \Theta_1^{newt} \simeq v_fl^{newt} \f$.
          */
          ppw->tca_T2_vector = 4./9.* zerokappam2/pvecthermo[pth->index_th_dkappa]*(y[pv->index_pt_theta_b] + y[pv->index_pt_V]);

          /* This is roughly 2.36 of 2410.03612 but otherwise it is 13 from 1012.0546.
             In both cases this is only given for flat space whereas for curved space we should use
             \f$ dv = -1/\kappa' * R/(1+R) * (a'/a*v_fl^{synchr} - 1/5*({}_0\kappa_2^m) \Theta_2) \f$
             with \f$ {}_0\kappa_2^m = \sqrt{3*(k^2-2K)} \f$.
             In principle the last term with the photon quadrupole is already tight-coupled
             and we should not put it.
             However it is the leading term when we have a vanishing baryon/photon fluid velocity,
             as is the case for octupolar initial conditions, see 3.31 of 2410.03612
             so we put to avoid a discontinuity when switching off tight-coupling approximation.
          */
          ppw->tca_slip_vector = -R_Hu/(1.+R_Hu)/pvecthermo[pth->index_th_dkappa]*(a_prime_over_a*y[pv->index_pt_theta_b] - zerokappam2/5*ppw->tca_T2_vector);

          /* We have added the correction to the slip coming from the first order quadrupole.
             This is second order in tight coupling, but the leading term when initial conditions
             have a vanishing initial baryon/photon fluid velocity (in synchronous gauge) */
          dy[pv->index_pt_theta_b] = -a_prime_over_a*R_Hu/(1.+R_Hu)*y[pv->index_pt_theta_b]  -zerokappam2/5./(1.+R_Hu)*ppw->tca_T2_vector;
          ...
          ppw->tca_slip_vector = -R_Hu/(1.+R_Hu)/pvecthermo[pth->index_th_dkappa]*(a_prime_over_a*(y[pv->index_pt_theta_b]-y[pv->index_pt_V]) - zerokappam2/5*ppw->tca_T2_vector);

          dy[pv->index_pt_theta_b] = -a_prime_over_a*R_Hu/(1.+R_Hu)*y[pv->index_pt_theta_b]
            + pvecmetric[ppw->index_mt_V_prime] +a_prime_over_a*R_Hu/(1.+R_Hu)*y[pv->index_pt_V]
            -zerokappam2/5./(1.+R_Hu)*ppw->tca_T2_vector;
