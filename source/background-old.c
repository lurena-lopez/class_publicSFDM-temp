...
int background_functions(
  ...
  if (pba->has_scf == _TRUE_) {
    ...
    if (pba->has_lambda == _TRUE_) {
          pvecback[pba->index_bg_rho_scf] = Omega_de*(rho_r+rho_m)/(1.-Omega_de) - pvecback[pba->index_bg_rho_lambda]; // energy of the scalar field
    }
    else{
...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_) {
      dy[pba->index_bi_Omega_phi_scf] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*y[pba->index_bi_Omega_phi_scf]*
        (pvecback[pba->index_bg_w_tot]+cosh(y[pba->index_bi_theta_phi_scf]));
      
      dy[pba->index_bi_Omega_de] =3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        ((1.+pvecback[pba->index_bg_w_tot])*y[pba->index_bi_Omega_de] -
      ...
      dy[pba->index_bi_Omega_de] =3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        ((1.+pvecback[pba->index_bg_w_tot])*y[pba->index_bi_Omega_de] -
         (1.-cosh(y[pba->index_bi_theta_phi_scf]))*exp(y[pba->index_bi_Omega_phi_scf])); */
